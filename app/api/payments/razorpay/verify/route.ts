...
export async function POST(req: Request) {
  try {
    ...
    const dbPayment = await prisma.payment.findFirst({ where: { stripeSessionId: razorpay_order_id } });
    if (dbPayment) {
      await prisma.payment.update({ where: { id: dbPayment.id }, data: { status: 'COMPLETED', creditAmount: Math.floor(payment.amount / 100) } });
    }
